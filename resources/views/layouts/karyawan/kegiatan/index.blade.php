...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('activities.apd.create') }}" class="btn btn-success">
                ...
                <tbody>
                    @foreach($activities as $index => $activity)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('activities.apd.show', $activity->id) }}" class="btn btn-info btn-sm">
                                    <i class="bi bi-eye"></i> Lihat
                                </a>
                                <a href="{{ route('activities.apd.edit', $activity->id) }}" class="btn btn-warning btn-sm">
                                    <i class="bi bi-pencil-square"></i> Edit
                                </a>
                                <form action="{{ route('activities.apd.destroy', $activity->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus kegiatan ini?')">
                                    @csrf
                                    @method('DELETE')
